...
$stmt = $mysqli->prepare( 'SELECT busMap.id as map_id, busMap.bus_id, busMap.cat_id, businesses.name as bus_name, Categories.name as cat_name FROM busMap INNER JOIN businesses on businesses.id = busMap.bus_id INNER JOIN Categories on Categories.id = busMap.cat_id WHERE cat_id = ? AND businesses.type = "repair"' );
